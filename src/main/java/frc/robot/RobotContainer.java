...
import frc.robot.commands.Player;
...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    this.m_driveSubsystem.setDefaultCommand((Command) new Teleop(this.m_driveSubsystem,
        () -> this.m_joystick.getRawAxis(1), () -> this.m_joystick.getRawAxis(3), logger));

    (new JoystickButton((GenericHID) this.m_joystick, 1))
        .whenPressed((Command) new ResetSensors(this.m_driveSubsystem));

    (new JoystickButton((GenericHID) this.m_joystick, 2))
        .whenHeld((Command) new Vomit(this.m_conveyor, this.m_collector));
  }
  ...
  public Command getAutonomousCommand() {
    ...
    return new Player(m_driveSubsystem);

    // Trajectory trajectory = null;
    // Supplier<Float> heading = () ->
    // Float.valueOf(this.m_driveSubsystem.GetCompass());

    // if (((Float) heading.get()).floatValue() >= 205.0F && ((Float)
    // heading.get()).floatValue() <= 215.0F) {
    // trajectory = this.builder.ReadTrajectorys("output/A-Red.wpilib.json");
    // System.out.println("Path A Red");
    // } else if (((Float) heading.get()).floatValue() >= 155.0F && ((Float)
    // heading.get()).floatValue() <= 165.0F) {
    // trajectory = this.builder.ReadTrajectorys("output/A-Blue.wpilib.json");
    // System.out.println("Path A Blue");
    // } else if (((Float) heading.get()).floatValue() >= 175.0F && ((Float)
    // heading.get()).floatValue() <= 185.0F) {
    // trajectory = this.builder.ReadTrajectorys("output/B-Blue.wpilib.json");
    // System.out.println("Path B Blue");
    // } else if (((Float) heading.get()).floatValue() >= 225.0F && ((Float)
    // heading.get()).floatValue() <= 235.0F) {
    ...
    // return new ChassisSpeeds(linearVelocityRefMeters, 0.0D,
    // angularVelocityRefRadiansPerSecond);
    // }
    // };
    // NetworkTableEntry m_leftReference =
    // NetworkTableInstance.getDefault().getTable("troubleshooting")
    // .getEntry("left_reference");
    // NetworkTableEntry m_leftMeasurement =
    // NetworkTableInstance.getDefault().getTable("troubleshooting")
    // .getEntry("left_measurement");
    // NetworkTableEntry m_rightReference =
    // NetworkTableInstance.getDefault().getTable("troubleshooting")
    // .getEntry("right_reference");
    // NetworkTableEntry m_rightMeasurement =
    // NetworkTableInstance.getDefault().getTable("troubleshooting")
    ...
    // new RamseteController(4.0D, 0.3D), this.m_driveSubsystem.getFeedForward(),
    // this.m_driveSubsystem.getKinematics(), this.m_driveSubsystem::getWheelSpeeds,
    // this.m_driveSubsystem.getLeftPIDController(),
    // this.m_driveSubsystem.getRightPIDController(),
    // (leftVolts, rightVolts) -> {
    // this.m_driveSubsystem.set(leftVolts.doubleValue(), rightVolts.doubleValue());
    // m_leftMeasurement.setNumber(Double.valueOf(this.m_driveSubsystem.getFeedForward()
    // .calculate((this.m_driveSubsystem.getWheelSpeeds()).leftMetersPerSecond)));
    // m_leftReference.setNumber(leftVolts);
    // m_rightMeasurement.setNumber(Double.valueOf(this.m_driveSubsystem.getFeedForward()
    // .calculate((this.m_driveSubsystem.getWheelSpeeds()).rightMetersPerSecond)));
    ...
    // return (Command) new ParallelCommandGroup(
    // new Command[] { (Command) new ConveyorAutomated(this.m_conveyor,
    // this.m_collector),
    // (Command) command.andThen(() -> this.m_driveSubsystem.set(0.0D, 0.0D), new
    // Subsystem[0]) });
  }
}
